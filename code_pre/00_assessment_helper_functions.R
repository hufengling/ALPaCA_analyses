...
get_threshold <- function(roc_obj, weighting = 3) {
  ...
  spec_ind <- which.max(roc_obj$sensitivities + weighting * roc_obj$specificities)
  spec <- roc_obj$thresholds[spec_ind]
  print(paste0("Spec: Sens = ", roc_obj$sensitivities[spec_ind], 
               " Spec = ", roc_obj$specificities[spec_ind]))
  sens_ind <- which.max(weighting * roc_obj$sensitivities + roc_obj$specificities)
...
get_train_roc <- function(preds_df) {
  ...
  return(list(les_roc = les_train_roc,
              ...
              plot(mimosa_train_roc, add = TRUE),
              plot(prl_train_roc, print.auc = TRUE),
              plot(aprl_train_roc, add = TRUE),
              plot(cvs_train_roc, print.auc = TRUE),
              plot(acvs_train_roc, add = TRUE)
  ))
}
