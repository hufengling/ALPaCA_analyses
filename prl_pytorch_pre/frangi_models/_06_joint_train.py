...
#bsub -q taki_normal -n 21 -J "train_predictor[1-10]" bash -c "python /home/fengling/Documents/prl/prl_pytorch/_06_joint_train.py"
import os
...
def isolate_lesion(patches_batch):
    ...
    for i in range(lesion_mask_tensor.size()[0]):
        ...
        if (len(lesion_ids) > 1): # Only need to blackout lesion if there are two lesions.
            ...
            mask_tensor[i, :, :, :, :] = (tmp_lesion_mask == 0) + (tmp_lesion_mask == id_to_keep)
        else:
        ...
        #if (len(lesion_ids) >= 1): # Only need to blackout lesion if there are two lesions.
        #    id_to_keep = lesion_ids[randrange(len(lesion_ids))]
        #    mask_tensor[i, :, :, :, :] = 0.1 + 0.9 * (tmp_lesion_mask == id_to_keep)
        #else:
        #    mask_tensor[i, :, :, :, :] = torch.ones_like(tmp_lesion_mask)
...
model_path = "/home/fengling/Documents/prl/prl_pytorch/cv_models_frangi/prl_autoencoder_05_" + str(prl_dl.i) + ".pt"
...
torch.save(prl_autoencoder.state_dict(), "/home/fengling/Documents/prl/prl_pytorch/cv_models_frangi/prl_joint_05_" + str(prl_dl.i) + ".pt")
torch.save(prl_predictor.state_dict(), "/home/fengling/Documents/prl/prl_pytorch/cv_models_frangi/prl_predictor_05_" + str(prl_dl.i) + ".pt")
