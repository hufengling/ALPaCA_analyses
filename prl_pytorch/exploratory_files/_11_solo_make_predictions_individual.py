...
lesion_autoencoder.load_state_dict(torch.load("/home/fengling/Documents/prl/prl_pytorch/cv_individual_models/lesion_joint_05_" + str(prl_dl.cv_index) + ".pt", map_location=torch.device('cpu')))
lesion_autoencoder.eval()
lesion_autoencoder.to(device)

# Load the state_dict from the checkpoint
prl_autoencoder.load_state_dict(torch.load("/home/fengling/Documents/prl/prl_pytorch/cv_individual_models/prl_joint_05_" + str(prl_dl.cv_index) + ".pt", map_location=torch.device('cpu')))
prl_autoencoder.eval()
prl_autoencoder.to(device)

# Load the state_dict from the checkpoint
cvs_autoencoder.load_state_dict(torch.load("/home/fengling/Documents/prl/prl_pytorch/cv_individual_models/cvs_joint_05_" + str(prl_dl.cv_index) + ".pt", map_location=torch.device('cpu')))
cvs_autoencoder.eval()
cvs_autoencoder.to(device)

# Load the state_dict from the checkpoint
lesion_predictor.load_state_dict(torch.load("/home/fengling/Documents/prl/prl_pytorch/cv_individual_models/lesion_predictor_05_" + str(prl_dl.cv_index) + ".pt", map_location=torch.device('cpu')))
lesion_predictor.eval()
lesion_predictor.to(device)

# Load the state_dict from the checkpoint
prl_predictor.load_state_dict(torch.load("/home/fengling/Documents/prl/prl_pytorch/cv_individual_models/prl_predictor_05_" + str(prl_dl.cv_index) + ".pt", map_location=torch.device('cpu')))
prl_predictor.eval()
prl_predictor.to(device)

# Load the state_dict from the checkpoint
cvs_predictor.load_state_dict(torch.load("/home/fengling/Documents/prl/prl_pytorch/cv_individual_models/cvs_predictor_05_" + str(prl_dl.cv_index) + ".pt", map_location=torch.device('cpu')))
cvs_predictor.eval()
cvs_predictor.to(device)
...
def extract_patch(coord, candidate_id, 
    ...
    frangi_patch = frangi[:, x_start:x_end, y_start:y_end, z_start:z_end]

    patch = torch.cat((t1_patch, flair_patch, epi_patch, phase_patch, frangi_patch))
    ...
    if rotate_patches:
        ...
        patch = torch.cat((rotate_patch(patch[0, :, :, :], invert, face, rotations).unsqueeze_(0),
                           ...
                           rotate_patch(patch[4, :, :, :], invert, face, rotations).unsqueeze_(0)))
...
def get_predictions(subject, num_coords):
    ...
    for candidate_id in range(1, int(lesion_mask.max()) + 1):
        ...
        with torch.no_grad():
            lesion_output = lesion_predictor(lesion_autoencoder.encoder(all_patch)) 
            prl_output = prl_predictor(prl_autoencoder.encoder(all_patch)) 
            cvs_output = cvs_predictor(cvs_autoencoder.encoder(all_patch))
        output = torch.cat((lesion_output, prl_output, cvs_output), dim = 1)
...
df = get_predictions(prl_dl.subject, 20)

if not os.path.exists("/home/fengling/Documents/prl/cv_output_individual/split_" + str(prl_dl.cv_index)):
    os.makedirs("/home/fengling/Documents/prl/cv_output_individual/split_" + str(prl_dl.cv_index), exist_ok = True)
    
df.to_csv("/home/fengling/Documents/prl/cv_output_individual/split_" + str(prl_dl.cv_index) + 
