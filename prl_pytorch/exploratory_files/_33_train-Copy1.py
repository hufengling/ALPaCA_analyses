...
def make_weight_tensor(patches_batch, isolation_mask, lesion_weight, epi_phase_weight):
    in_lesion = (patches_batch["lesion_mask"]["data"] != 0) * isolation_mask[:, 0:1, :, :, :]
    weight_tensor = in_lesion * (lesion_weight - 1) + 1
    weight_tensor = weight_tensor.repeat(1, 4, 1, 1, 1)
    weight_tensor[:, 2:3, :, :, :] = weight_tensor[:, 2:3, :, :, :] * epi_phase_weight
...
for epoch_index in range(num_epochs):
    ...
    for patches_batch in prl_dl.train_loader:
        ...
        weight_tensor = make_weight_tensor(patches_batch, isolation_mask_tensor, 10, 5) # Upweight reconstruction of lesions by 10:1. Upweight reconstruction of primary lesion by 5:1
