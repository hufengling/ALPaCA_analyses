...
count_missing_lesions <- function(i, data_dir) {
  ...
  return(cbind(subject[i], 
               as.data.frame(c(contains_lesions, contains_cvs, lesion_missing, prl_missing, cvs_missing))))
}

missing_lesions <- as.data.frame(matrix(rep(0, 8 * 97), ncol = 8, nrow = 97))
for (i in 1:97) {
  missing_lesions[i, ] <- count_missing_lesions(i, data_dir)
}

missing_sum <- colSums(missing_lesions, na.rm = TRUE); missing_sum
